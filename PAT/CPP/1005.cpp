...
int findkey(int *numlist,int k,int *keylist){
	int num,othernum,*nokeylist,m=0;
	nokeylist=new int[k];
	for(int i=0;i<k;i++){
		int flag=0;
		for(int j=0;j<k;j++){
			...
			if (numlist[j]!=numlist[i]){
				while(othernum>1){
					if(othernum%2==0){
						othernum=othernum/2;
						if (othernum==numlist[i]){
							flag=1;
						}
					}else{
						othernum=(othernum*3+1)/2;
						if (othernum==numlist[i]){
							flag=1;
						}
					}
...
void bubblesort(int *keylist,int m){
...
int main(){
	...
	m=findkey(numlist,k,keylist);
	bubblesort(keylist,m);
	for(int j=0;j<m-1;j++){
			cout<<keylist[j]<<" ";
	}
	cout<<keylist[m-1];
